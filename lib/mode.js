      ...
      for (let input of classDefinition.inputs) {
        ...
        if (input.required && !(values?.[0])) {
          ...
          for (let val of values) {
            ...
            // A hack to ignore type of select, selectobject and selecturl
            const ignoreType = t => t.toLowerCase() in { select: 1, selectobject: 1, selecturl: 1 };
            let typeFound = input.type.some(
              val['@id'] ?
                (type) => ignoreType(type) || val['@type'].includes(type) :
